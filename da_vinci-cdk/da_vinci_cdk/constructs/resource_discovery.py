...
class DiscoverableResourceDynamoDBLookup(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, resource_name: str, resource_type: str, table_name: str):
        ...
        self.registration = ResourceRegistration(
            endpoint="PLACEHOLDER",  # Placeholder, not used for lookup but is required by the Class
            resource_name=resource_name,
            resource_type=resource_type,
        )

        self.registration.gen_dynamodb_key()
        
        self.full_table_name = resource_namer(name=table_name, scope=self)
        
        # Get the partition and sort key values
        partition_key_value = self.registration.attribute_value(
            self.registration.partition_key_attribute.name
        )
        
        sort_key_value = self.registration.attribute_value(
            self.registration.sort_key_attribute.name
        )
        
        # Generate the DynamoDB key
        self.item_key = self.registration.gen_dynamodb_key(
            partition_key_value=partition_key_value,
            sort_key_value=sort_key_value,
        )
    ...
    def _get_item_call(self) -> AwsSdkCall:
        ...
        return AwsSdkCall(
            ...
            physical_resource_id=PhysicalResourceId.of(
                f'{self.registration.resource_type}-{self.registration.resource_name}-lookup'
            )
        )
